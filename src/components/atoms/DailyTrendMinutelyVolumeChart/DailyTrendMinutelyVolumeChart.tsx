...
function DailyTrendMinutelyVolumeChart() {
  ...
  useEffect(() => {
    ...
    const getMon = axios.get<Response>(`http://localhost:8080/api/minutely/stacked/volume/companies/${code}`, {
      params: {
        'date': moment().subtract(6, 'days').format('YYYY-MM-DD')
      }
    });
    const getTue = axios.get<Response>(`http://localhost:8080/api/minutely/stacked/volume/companies/${code}`, {
      params: {
        'date': moment().subtract(5, 'days').format('YYYY-MM-DD')
      }
    });
    const getWed = axios.get<Response>(`http://localhost:8080/api/minutely/stacked/volume/companies/${code}`, {
      params: {
        'date': moment().subtract(4, 'days').format('YYYY-MM-DD')
      }
    });
    const getThu = axios.get<Response>(`http://localhost:8080/api/minutely/stacked/volume/companies/${code}`, {
      params: {
        'date': moment().subtract(3, 'days').format('YYYY-MM-DD')
      }
    });
    const getFri = axios.get<Response>(`http://localhost:8080/api/minutely/stacked/volume/companies/${code}`, {
      params: {
...
export default DailyTrendMinutelyVolumeChart;
