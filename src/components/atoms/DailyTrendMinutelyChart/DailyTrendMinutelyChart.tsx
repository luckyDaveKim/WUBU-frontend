...
function DailyTrendMinutelyChart() {
  ...
  useEffect(() => {
    ...
    const getMon = axios.get<Response>(`http://localhost:8080/api/minutely/price/${monDate}/companies/${code}`);
    const tueDate = moment().subtract(5, 'days').format('YYYY-MM-DD')
    const getTue = axios.get<Response>(`http://localhost:8080/api/minutely/price/${tueDate}/companies/${code}`);
    const wedDate = moment().subtract(4, 'days').format('YYYY-MM-DD')
    const getWed = axios.get<Response>(`http://localhost:8080/api/minutely/price/${wedDate}/companies/${code}`);
    const thuDate = moment().subtract(3, 'days').format('YYYY-MM-DD')
    const getThu = axios.get<Response>(`http://localhost:8080/api/minutely/price/${thuDate}/companies/${code}`);
    const friDate = moment().subtract(2, 'days').format('YYYY-MM-DD')
    const getFri = axios.get<Response>(`http://localhost:8080/api/minutely/price/${friDate}/companies/${code}`);
